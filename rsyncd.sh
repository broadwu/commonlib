...
# 实现chkconfig来管理开启自启动， 2345是Linux运行级别
if [ $# -ne 1 ]              #<==如果传参的个数不等于
...
then    
    ...
    sleep 2              #<==休息2秒，这点很重要，停止及启动服务后，建议先休息1~2                            秒，再判断。
    
    if [ `netstat -lntup|grep rsync|wc -l` -ge 1 ]#<==如果过滤rsync字符串的行数大于1，则表示rsync服务启动了。
      then        
...
then    
    ...
    if [ `netstat -lntup|grep rsync|wc -l` -eq 0 ]#<==如果过滤rsync字符串的行数为0，则表示rsync服务停止了。
    then        
